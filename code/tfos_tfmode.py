...
def inference(args, ctx):
  ...
  def parse_tfr(example_proto):
    ...
    image = tf.cast(features['image'], dtype=tf.float32) / 255.0  #轉換型態
    image = tf.reshape(image, [28, 28, 1])
    label = tf.cast(features['label'], dtype=tf.float32)   #轉換型態
    return (image, label)

  # define a new tf.data.Dataset (for inferencing)，主要是在讀取MNIST
